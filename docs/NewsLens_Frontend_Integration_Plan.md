...
* Matches snapshots using:
  ...
  ```ts
  snapshot.sourceId === source._id && snapshot.id.endsWith(`-${slot.id}`)
  ```
...
* `snapshot.id` is generated by the backend as `${sourceId}-${timeSlot}` and is safe to rely on for all grid-matching logic.
...
## 🎛️ Component Integration: Controls (Sidebar)
